...
public class EmoteDisplayer {
        ...
        if (cooldownSeconds > 0) {
            ...
            if (remaining > 0) {
                String msg = ChatColor.translateAlternateColorCodes('&',
                                plugin.getConfig().getString("cooldown.message", "&cPlease wait %seconds% second(s) before sending another emoji."))
    ...
    private static void syncChat(EmojiManager plugin, Player player, EmoteManager.Emote emote) {
        ...
        String template = plugin.getConfig().getString("chat-sync.message", "&e%player% &rused emoji %symbol%");
        String msg = template
