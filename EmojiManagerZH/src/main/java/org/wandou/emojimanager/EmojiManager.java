...
public final class EmojiManager extends JavaPlugin {
    ...
    public void onEnable() {
        saveDefaultConfig();
        saveResource("mysql.yml", false);   // 新增：生成 MySQL 配置文件
        ...
        playerDataManager = new PlayerDataManager(this, databaseManager);   // 传入数据库管理器
        ...
        getLogger().info("EmojiManager 1.2 已启用！当前存储方式: " +
    ...
    public void onDisable() {
        if (databaseManager != null) {
            databaseManager.disconnect();   // 关闭数据库连接
        }
        getLogger().info("EmojiManager 1.2 已禁用。");
    }
}